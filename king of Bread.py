...
def drawLevel(matrix_to_draw):
	...
	# Load level images
	prateleira = pygame.image.load(myEnvironment.getPath() + '/themes/' + theme + '/images/prateleira.png').convert()
	box = pygame.image.load(myEnvironment.getPath() + '/themes/' + theme + '/images/box.png').convert()
	cortar_massa = pygame.image.load(myEnvironment.getPath() + '/themes/' + theme + '/images/cortar_massa.png').convert()
	forno = pygame.image.load(myEnvironment.getPath() + '/themes/' + theme + '/images/forno.png').convert()
	massa = pygame.image.load(myEnvironment.getPath() + '/themes/' + theme + '/images/massa.png').convert()
	...
	if myLevel.getSize()[0] > myEnvironment.size[0] / 36 or myLevel.getSize()[1] > myEnvironment.size[1] / 36:
		...
		prateleira = pygame.transform.scale(prateleira, (new_image_size,new_image_size))
		forno = pygame.transform.scale(forno, (new_image_size,new_image_size))
		massa = pygame.transform.scale(massa, (new_image_size,new_image_size))
		cortar_massa = pygame.transform.scale(cortar_massa, (new_image_size,new_image_size))
	...
	images = {'%': cortar_massa,'&': forno,'!': massa,'#': prateleira, ' ': space, '$': box, '.': target, '@': player, '*': box_on_target}
	
	# Get image size. Images are always squares so it doesn't care if you get width or height
	box_size = prateleira.get_width()
...
def movePlayer(direction,myLevel):
	...
	if direction == "L":
		...
		# if is_box_on_target
			...
			print ("Ingrediente na forma encontrado")
			if matrix[y][x-2] == " ":
				matrix[y][x-2] = "$"
				matrix[y][x-1] = "@"
				if target_found == True:
	...
	# if is_box_on_target % = Corta Massa
		elif matrix[y][x-1] == "*":
			print ("Ingrediente na forma encontrado")
			if matrix[y][x-2] == " ":
				matrix[y][x-2] = "%"
	...
	print ("Ingredietes restantes: " + str(len(myLevel.getBoxes())))
